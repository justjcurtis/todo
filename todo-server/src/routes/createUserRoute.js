...
const checkPassword = (password) => {
    ...
    const has1Symbol = password.match(/[!@#$%^&*]/);
    return has2Numbers && has2Capitals && has1Symbol && password.length >= 8;
}
...
const createUserRoute = async (req, res) => {
    ...
    if (checkPassword(password)) return res.status(400).json({ error: 'Password must be at least 8 characters with at leat 2 numbers 2 capital letter and 1 symbol' });
